    ...
    for j in range(number_of_epoch):
        ...
        for i in range(number_of_batches):
            ...
            neural_network.backward_pass(
                train_cases[1][train_batche_lentgh*i:train_batche_lentgh*(i+1)])
            validation_loss = neural_network.forward_pass(
                validation_size, validation_cases[0], validation_cases[1])
            validation_losses.append(validation_loss)

    # test cases
    test_loss = neural_network.forward_pass(
        test_size, test_cases[0], test_cases[1])
    test_losses = np.ones(test_size)*test_loss

    x1 = np.linspace(0, number_of_batches*number_of_epoch -
                     1, number_of_batches*number_of_epoch)
    x2 = np.linspace(0, number_of_batches*number_of_epoch -
                     1, number_of_batches*number_of_epoch)
    x3 = np.linspace(number_of_batches*number_of_epoch,
                     number_of_batches*number_of_epoch+10, test_size)
...
train_cases, validation_cases, test_cases, train_cases_count, validation_cases_count, test_cases_count = generator(
    count=count, n=n, _doodle_image_types_=_doodle_image_types_)
...
fit(N, number_of_batches, number_of_epoch, train_cases, train_cases_count,
    validation_cases, validation_cases_count, test_cases, test_cases_count)
