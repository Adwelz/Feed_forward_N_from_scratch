...
class Layer:
    ...
    def backward_pass(self, jacobian_l_z_list, wrt='none', wreg='0'):
        ...
        if wrt == 'L2':
            jacobian_l_w = jacobian_l_z_list[0] * \
                simp_j_z_w+float(wreg)*self.weights

        if wrt == 'L1':
            jacobian_l_w = jacobian_l_z_list[0] * \
                simp_j_z_w+float(wreg)*np.sign(self.weights)
        ...
        for i in range(1, len(self.inputs)):
            ...
            jacobian_l_y_list.append(
                np.dot(jacobian_l_z_list[i], jacobian_z_y))
            ...
            if wrt == 'L2':
                jacobian_l_w += jacobian_l_z_list[i] * \
                    simp_j_z_w+float(wreg)*self.weights
        ...
        # update the biais
        self.biais = self.biais-self.b_lr * \
            np.sum(jacobian_l_z_list, axis=0)/len(jacobian_l_z_list)
